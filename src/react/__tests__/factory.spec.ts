...
import { isFunction, pipe } from "../../utilities";
import { createARQFactory, ARQFactory } from "../factory";
import { act, renderHook, waitFor } from "@testing-library/react";
import { createARQContext, ARQProvider } from "../context";
import {
  createElement,
  StrictMode,
  type FC,
...
import { type ExtractSchemaType } from "../../type";
...
describe("Tests", () => {
  ...
  const TestARQProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const TestARQProviderWithStrictMode: FC<PropsWithChildren> = ({
    children,
  }) => {
    return createElement(
      StrictMode,
      null,
      createElement(TestARQProvider, {}, children),
    );
  };
  const useGETForGreet = createARQFactory("GET:/api/greet")
    .apply<"Response", "hi!">()
    .makeQuery();
  const useGETForUsers = createARQFactory("GET:/api/users/list")
    .apply<"Response", { list: User[]; total: number }>()
    .apply<"Query", { limit: number; page: number; keywords?: string }>()
    .makeQuery();
  const useGETForUserProfile = createARQFactory(
    "GET:/api/users/{userId}/profile",
  )
    .apply<"Response", User>()
    .makeQuery();
  const usePOSTForCreateUser = createARQFactory("POST:/api/users/create")
    .apply<"Response", User["id"]>()
    .makeMutation();
  const usePUTForUpdateUser = createARQFactory("PUT:/api/users/{userId}")
    .apply<"Response", User["id"]>()
  ...
  it("should get data and return type test in StrictMode", async () => {
    ...
    const { result } = renderHook(
      ...
      { wrapper: TestARQProviderWithStrictMode },
    );
  ...
  it("should only be triggered when enabled", async () => {
    ...
    const { result, rerender } = renderHook(
      ...
      { initialProps: false, wrapper: TestARQProvider },
    );
  ...
  it("should expire normally cache", async () => {
    ...
    expect(client.queries.isStale(`:http+${id}`)).toBeFalsy();
    // wait cache expired
    await wait(300);
    expect(client.queries.isStale(`:http+${id}`)).toBeTruthy();
    // update user
    await act(async () => {
  ...
  it("should not re-fetch when query params are deeply equal", async () => {
    const { result, rerender } = renderHook(
      ({ query }) => {
        return useGETForUsers({
          query,
      ...
      },
      {
        ...
        initialProps: {
          query: {
            limit: 1,
            page: 1,
            keywords: HttpMocker.resources[0].name,
          },
        },
      },
    );
  ...
  // type test case
  it("should normal use ExtractSchemaType to infer type", () => {
    expectTypeOf<
      ExtractSchemaType<typeof useGETForGreet, "Response">
    >().toEqualTypeOf<"hi!">();
    expectTypeOf<
      ExtractSchemaType<typeof useGETForGreet, "Response">
    >().not.toEqualTypeOf<{}>();
    expectTypeOf<
      ExtractSchemaType<typeof useGETForGreet, "Headers">
    >().toEqualTypeOf<unknown>();
    expectTypeOf<
      ExtractSchemaType<typeof useGETForGreet, "Query">
    >().toEqualTypeOf<unknown>();
    expectTypeOf<
      ExtractSchemaType<typeof useGETForGreet, "Error">
    >().toEqualTypeOf<unknown>();
    expectTypeOf<
      ExtractSchemaType<typeof useGETForGreet, "Body">
    >().toEqualTypeOf<unknown>();
    expectTypeOf<
      ExtractSchemaType<typeof useGETForUsers, "Response">
    >().toEqualTypeOf<{ list: User[]; total: number }>();
    expectTypeOf<
      ExtractSchemaType<typeof useGETForUsers, "Query">
    >().toEqualTypeOf<{
      limit: number;
      page: number;
      keywords?: string;
    ...
    expectTypeOf<
      ExtractSchemaType<typeof useGETForUserProfile, "Response">
    >().toEqualTypeOf<User>();
    expectTypeOf<
      ExtractSchemaType<typeof usePOSTForCreateUser, "Response">
    >().toEqualTypeOf<string>();
    expectTypeOf<
      ExtractSchemaType<typeof usePUTForUpdateUser, "Body">
